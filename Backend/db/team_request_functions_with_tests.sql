...
DROP TRIGGER IF EXISTS team_requests_updated_at_trigger ON team_requests;
...
-- Step 1: Get some real IDs from your database
-- Uncomment and run these to find IDs to use:
/*
SELECT project_id FROM projects LIMIT 1;
SELECT user_id FROM users LIMIT 3;
SELECT project_id, guide_id FROM projects WHERE project_id = (SELECT project_id FROM projects LIMIT 1);
*/

-- Step 2: Example test calls (replace <<ID>> with actual IDs)
-- Test 1: Create team request
/*
DO $$
DECLARE
    v_project_id integer := <<existing_project_id>>;
    v_leader_id integer := <<leader_user_id>>;
    v_member1_id integer := <<member1_user_id>>;
    v_member2_id integer := <<member2_user_id>>;
    v_request_id integer;
BEGIN
...
/*
SELECT public.team_request_member_reply(<<request_id>>, <<member1_user_id>>, 'accepted');
*/

-- Test 3: Member 2 accepts
/*
SELECT public.team_request_member_reply(<<request_id>>, <<member2_user_id>>, 'accepted');
*/

-- Test 4: Leader accepts (should trigger team creation)
/*
SELECT public.team_request_member_reply(<<request_id>>, <<leader_user_id>>, 'accepted');
*/

-- Test 5: Check status and team creation
/*
SELECT request_id, status FROM team_requests WHERE request_id = <<request_id>>;
SELECT team_id, team_name FROM teams WHERE project_id = <<project_id>>;
SELECT user_id, role_in_team FROM team_members WHERE team_id = (SELECT team_id FROM teams WHERE project_id = <<project_id>>);
*/

-- Test 6: Guide decision (if guide exists)
/*
SELECT public.team_request_guide_decision(<<request_id>>, <<guide_user_id>>, 'approve');
*/

-- Test 7: Check final status
/*
SELECT request_id, status FROM team_requests WHERE request_id = <<request_id>>;
SELECT project_id, status, guide_status, visibility, ispublished FROM projects WHERE project_id = <<project_id>>;
*/
